...
import { type ConflictDetectionOutput } from '@/ai/flows/ai-conflict-detection-flow';
import { cn } from '@/lib/utils';

interface ConflictSummaryProps {
  analysis: ConflictDetectionOutput | null;
  isAnalyzing: boolean;
}

export default function ConflictSummary({ analysis, isAnalyzing }: ConflictSummaryProps) {
  if (isAnalyzing) {
    return (
      <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/5 animate-pulse flex flex-col items-center justify-center gap-4">
  ...
  if (!analysis.hasConflicts) {
    return (
      <div className="p-6 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex gap-4 items-start">
        ...
        {analysis.conflicts.map((conflict, idx) => (
          <div 
            ...
            className={cn(
              ...
              conflict.type === 'overlap' ? "bg-red-500/10 border border-red-500/20" : "bg-primary/10 border border-primary/20"
            )}
          >
